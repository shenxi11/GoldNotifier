...
服务端开始保存获取到的新鲜行情点和每日汇总，供 App 或外部分析方查询趋势，并为后续日 K 提供每日 OHLC 基础数据。当前版本沿用已有 Redis，不新增数据库。
...
- 历史明细 key 以 `gold:history:{SYMBOL}:{YYYY-MM-DD}` 命名，默认保留 2 天，用于短周期趋势和 K 线聚合。
- 服务端会同步维护每日汇总 key `gold:daily_summary:{SYMBOL}:{YYYY-MM-DD}`，保存当天 `open/high/low/close`，默认保留 3650 天。
- 每日汇总保留期由 `DAILY_SUMMARY_RETENTION_DAYS` 控制，和 `HISTORY_RETENTION_DAYS` 拆分，避免长期保存高频明细。
- 如果每日汇总 key 缺失，服务端会从已有历史行情回填汇总，兼容部署前已经写入的历史数据。
- 读取已有每日汇总时会刷新为长期 TTL，部署前短 TTL 的 key 会在被 `/latest` 或汇总读取逻辑命中后自动续期。
...
- 每日汇总长期保留在 Redis 中，适合支撑后续日 K 的 OHLC 数据源；如果后续要做多年级别分析、备份或跨机器迁移，仍需要评估数据库或 Redis 持久化策略。
